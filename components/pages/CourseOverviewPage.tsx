...
export default function CourseOverviewPage({ courseId }: { courseId: ParamValue }) {
        ...
        <div className="py-5 px-4 sm:py-10 bg-gray-100 min-h-screen">
          <div className="bg-violet-800 text-white rounded-2xl overflow-hidden mb-6 w-full sm:max-w-3xl sm:mx-auto">
            <div className="flex items-center px-4 py-3 border-b border-white/40">
              <button onClick={() => router.push('/courses')} className="text-lg sm:text-xl mr-4 cursor-pointer">←</button>
          ...
          {course.map((module, index) => (
            <div key={index} className="mb-20 mx-auto max-w-3xl">
              ...
              <div className="mt-10 flex flex-col items-center gap-4 px-4 sm:px-0">
                {module.cards.map((card: any, i: number) => {
                  ...
                  return (
                    <div key={i} className={`w-full flex flex-col ${i % 2 === 0 ? "items-start" : "items-end"}`}>
                      <div className={`flex flex-col items-center w-1/6 ${i % 2 === 0 ? 'ml-0 sm:ml-41.25' : 'sm:mr-41.25'}`}>
                        <button
                          onClick={() => {
                            if (isClickable) {
        ...
        <div className="fixed inset-0 z-20 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded-2xl shadow-xl w-125 space-y-4">
            ...
            <div className="flex justify-end pt-4">
              <button
                onClick={() => { setWarning(""); setLoading(false); router.push("/courses"); }}
