...
const HomePage = () => {
  ...
  const [finScoreLog, setFinScoreLog] = useState<any[]>([])
  ...
  const fetchLeaderboard = async () => {
    ...
    try {
      ...
      toast.error(`Failed to load leaderboard. Error: ${err}`);
      setShowLeaderBoard(false);
    } finally {
      setLoading(false);
    }
  ...
  const fetchFinScoreLog = async () => {
    ...
    try {
      ...
      toast.error(`Failed to load fin score history. Error: ${err}`);
  ...
  return (
    <div className="bg-gray-100 font-inter text-[#1e1e1e] pb-5 2xl:max-w-500 2xl:mx-auto">
        ...
        <div className="p-4 animate-pulse space-y-6 2xl:max-w-350 2xl:mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 items-start pt-6">
            <div className="space-y-4 w-full">
              <div className="bg-gray-300 rounded-2xl w-full h-43.5" />
              <div className="bg-gray-300 rounded-2xl w-full h-27.5" />
            </div>
            <div className="w-full h-74 bg-gray-300 rounded-2xl" />
            <div className="w-full h-74 bg-gray-300 rounded-2xl" />
          </div>
          <div className="flex flex-col sm:flex-row gap-6 pt-8">
            <div className="flex-1 space-y-4">
              ...
              <div className="flex gap-4 overflow-hidden bg-gray-300 rounded-2xl">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="w-77.5 h-87.5 bg-gray-300 rounded-2xl" />
                ))}
              </div>
            </div>
            <div className="w-full sm:w-105 h-97.5 bg-gray-300 rounded-2xl" />
          </div>
        </div>
      ) : (
        <div className="pt-5 px-4 sm:px-10 2xl:max-w-350 2xl:mx-auto">
          <main className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 items-start bg-gray-100 mb-5">
            <div className="col-span-1 md:col-span-1 xl:col-span-1">
              ...
              <section className="flex items-center bg-white rounded-2xl p-2 gap-4 border border-gray-300 mt-4">
                ...
                <div className="flex flex-col justify-center items-center grow gap-1 sm:gap-2">
                  ...
                  <button
                    onClick={() => router.push(`/courses/course/${ongoingCourse?.id || recommendedCourses[recommendedCourses.length - 1]?.id}`)}
            ...
            <section onClick={() => router.push("/articles")} className="bg-white px-4 sm:px-5 py-2 rounded-2xl font-sans flex flex-col justify-between w-full h-80 border border-gray-300 col-span-1 md:col-span-1 xl:col-span-1 cursor-pointer">
            ...
            <section
              ...
              className="bg-white rounded-2xl px-4 py-2 text-center flex flex-col flex-1 w-full max-w-120 mx-auto h-80 border border-gray-300 cursor-pointer overflow-hidden col-span-1 md:col-span-1 xl:col-span-1"
            >
          ...
          <div className="flex flex-col xl:flex-row gap-5 pb-8 items-start bg-gray-100">
            <div className="w-full xl:w-2/3">
              ...
              <div
                ...
                className="carousel-track bg-white rounded-2xl flex overflow-x-auto max-w-77.5 sm:max-w-155 md:max-w-232.5 xl:max-w-231.75 2xl:max-w-350 mx-auto border border-gray-300 snap-x snap-mandatory gap-4 sm:gap-3 px-4 sm:px-0"
              >
              ...
              <section className="bg-white rounded-3xl border border-gray-300 text-center flex flex-col justify-between w-full min-h-80 sm:h-110 mt-4 xl:mt-0">
                ...
                <div className="border-t border-gray-300 py-3">
                  <div
                    onClick={() => {
                      if (course_id) {
                        router.push(`/policies?courseId=${course_id}`);
                      } else {
                  ...
                  >
                    ...
                    <span className="text-2xl -mt-0.75">→</span>
                  </div>
                </div>
              </section>
            </div>
      ...
      )}
      {
        warning && (
          <div className="fixed inset-0 z-20 bg-black/40 flex items-center justify-center">
            <div className="bg-white p-6 rounded-2xl shadow-xl w-[90%] max-w-125 space-y-4">
...

      {
        error && (
          <div className="fixed inset-0 z-20 bg-black/40 flex items-center justify-center">
            <div className="bg-white p-4 rounded-2xl shadow-xl w-[90%] max-w-125 space-y-4">
              <p className="text-lg sm:text-xl font-bold text-red-600">⚠️ Alert</p>
        ...
        showLeaderBoard && (
          <div onClick={() => setShowLeaderBoard(false)} className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
            <div onClick={(e) => e.stopPropagation()} className="bg-white w-[90%] max-w-xl rounded-2xl shadow-xl p-6 relative">
              ...
              {loading ? (
                <div className="overflow-y-auto max-h-100 divide-y divide-gray-100 px-4">
                  {[...Array(6)].map((_, index) => (
...

      {
        showFinScoreLog && (
          <div onClick={() => setShowFinScoreLog(false)} className="fixed inset-0 z-20 bg-black/40 flex items-center justify-center">
            <div onClick={(e) => e.stopPropagation()} className="bg-white px-4 sm:px-6 py-4 rounded-2xl shadow-xl w-[90%] max-w-125 space-y-4">
