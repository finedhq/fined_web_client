...
export default function LandingPage() {
  ...
  const [openedArticle, setOpenedArticle] = useState<any>(null)
  ...
  const checkScroll = (el: any, setLeft: any, setRight: any) => {
    if (!el) return;
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-5 text-gray-800 font-inter">
      ...
      <section className="bg-[#3B0DAD] text-white py-12 sm:py-16 px-6 sm:px-10 lg:px-16 text-center relative">
        ...
        <Link href={`/courses/course/${courses[courses.length - 1]?.id}`} className="bg-[#fbbf24] text-white py-3 px-8 rounded-lg font-bold no-underline text-base sm:text-lg hover:bg-[#e6b640] transition-colors duration-200">Give It a Go →</Link>
      </section>
      ...
      <div className="py-12 sm:py-16 px-6 sm:px-10 lg:px-20 flex flex-col md:flex-row justify-between items-center bg-gray-100 gap-8">
        ...
        <div className="flex-1 w-full md:w-[45%] flex justify-center">
          <img
            ...
            className="w-full max-w-125 h-auto rounded-lg shadow-lg object-contain"
          />
        </div>
      </div>
      ...
      <div className="px-6 sm:px-10 lg:px-20 flex flex-col-reverse md:flex-row justify-between items-center bg-gray-100 gap-8">
        <div className="flex-1 w-full md:w-[45%] flex justify-center mb-10 md:mb-0">
          <img
            ...
            className="w-full max-w-150 h-auto rounded-xl shadow-lg object-contain"
          />
        </div>
      ...
      <section className="wave-container text-white py-16 px-6 sm:px-10 lg:px-16">
        ...
        <div ref={courseCarouselRef} role="region" aria-label="Explore courses carousel" className="flex flex-row h-60 pt-4 gap-8 sm:px-2 mb-10 overflow-x-auto snap-x snap-mandatory hide-scrollbar">
          {courses.length > 0 ? (
            courses.map((course, index) => (
              <div onClick={() => toast.error("Please sign in!")} key={index} className="bg-gray-100 text-gray-900 rounded-xl p-4 shadow-md flex flex-col justify-between h-48 w-full sm:w-110 shrink-0 transition-transform duration-200 hover:-translate-y-1 snap-start card-content cursor-pointer">
      ...
      <section className="bg-gray-100 py-16 px-6 sm:px-10 lg:px-16">
        ...
        <div ref={articleCarouselRef} role="region" aria-label="Explore courses carousel" className="flex flex-row h-47.5 pt-4 gap-8 sm:px-2 mb-10 overflow-x-auto snap-x snap-mandatory hide-scrollbar">
          {articles.length > 0 ? (
            articles.map((article, index) => (
              <div onClick={() => setOpenedArticle(article)} key={index} className="bg-gray-50 text-gray-900 rounded-xl p-4 shadow-lg flex flex-col justify-between h-40 w-full sm:w-110 shrink-0 transition-transform duration-200 hover:-translate-y-1 snap-start card-content cursor-pointer">
                <div className='flex justify-between gap-4' >
                  <div className='max-w-3/5' >
