...
const getPoints = (index: number) => index + 1; // A=1, B=2, C=3, D=4 

const getFinancialProfile = (totalScore: number) => {
...
const ModuleContentPage = ({ params }: { params: Params }) => {
  ...
  const { courseId, moduleId, cardId } = params;
  ...
  const [prevModuleCard, setPrevModuleCard] = useState<any>(null)
  const [nextModuleCard, setNextModuleCard] = useState<any>(null)
  const [loading, setLoading] = useState(true)
  const [progressPercent, setProgressPercent] = useState(0)
  const [warning, setWarning] = useState("")

  const [localCompletedCards, setLocalCompletedCards] = useState<Record<string, any>>({})
  console.log('local: ', localCompletedCards)
  ...
  async function fetchCard() {
    try {
      ...
      if (
        ...
        const selectedIdx = fetchedCard.options.findIndex((opt: any) => opt === fetchedCard.userAnswer)
  ...
  async function markCompleted(userAnswer = null, userIndex: number | null | undefined = null) {
    try {
      ...
      if (!nextCardId && !nextModuleCard && isLastCardInModule) {
        ...
        // setTimeout(() => {
        //   router.push(`/home/?courseId=${courseId}`)
        // }, 1500)
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (card.status !== "completed" && isAuthenticated) {
      ...
      setLocalCompletedCards(prev => ({ ...prev, [cardId as string]: true }))
    }
  }, [card?.card_id, email])

  async function checkIsCorrect(index: number) {
    if (disabled) return;
        ...
        card?.content_type === "text" || card?.content_type === "image" ? (
          <div>
            <div className='flex items-center gap-4 mb-6' >
              <RxCross2 onClick={() => router.push(`/courses/course/${courseId}`)} className='text-2xl cursor-pointer' />
            ...
            <div className="flex justify-between mt-8">
              ...
              {prevCardId ? (
                <button
                  onClick={() =>
                    router.push(`/courses/course/${courseId}/module/${moduleId}/card/${prevCardId}`)
                  }
                ...
                <button
                  onClick={() =>
                    router.push(`/courses/course/${courseId}/module/${prevModuleCard.moduleId}/card/${prevModuleCard.cardId}`)
                  }
              ...
              {nextCardId ? (
                (isAuthenticated ? card.status === "completed" : localCompletedCards[cardId as string]) ? (
                  <button
                    onClick={() =>
                      router.push(`/courses/course/${courseId}/module/${moduleId}/card/${nextCardId}`)
                    }
                    className="bg-yellow-400 hover:bg-yellow-500 text-white px-6 py-2 rounded-full flex items-center gap-2 cursor-pointer"
                  >
                    Next <FaArrowRight />
                ...
                <button
                  onClick={() =>
                    router.push(`/courses/course/${courseId}/module/${nextModuleCard.moduleId}/card/${nextModuleCard.cardId}`)
                  }
                  className="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-full flex items-center gap-2 cursor-pointer"
                >
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl text-center">
            ...
            <div className="text-left mb-8">
              ...
              <ul className="space-y-2">
                {finalProfile.products.map((p: string, i: number) => (
        ...
        <div className="fixed inset-0 z-20 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded-2xl shadow-xl w-125 space-y-4">
            ...
            <div className="flex justify-end pt-4">
              <button
                onClick={() => { setWarning(''); router.push("/courses") }}
