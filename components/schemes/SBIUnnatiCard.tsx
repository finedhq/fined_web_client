...
const UnnatiCardList = () => {
  const { getSBIUnnatiCard, unnaticard } = useMessagesStore();
    ...
    switch (tab) {
      ...
      case "Fees and Charges":
        return Array.isArray(product.fees_and_charges) ? (
          <ul className="list-disc pl-5 space-y-1">
            {product.fees_and_charges.map((fee, i) => (
              <li key={i}>{fee}</li>
            ))}
          </ul>
        ) : (
