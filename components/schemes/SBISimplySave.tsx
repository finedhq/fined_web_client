  ...
  const getTabContent = (product, tab) => {
    ...
    switch (tab) {
      ...
      case "Key Features":
        return Array.isArray(product.features) ? (
          <ul className="list-disc pl-5 space-y-2">
            {product.features.map(([key, value], i) => (
              <li key={i}>
      ...
      case "Fees and Charges":
        return Array.isArray(product.fees_and_charges) ? (
          <ul className="list-disc pl-5 space-y-2">
            {product.fees_and_charges.map(([key, value], i) => (
              <li key={i}>
                <strong>{key}:</strong> {value}
              </li>
            ))}
            ...
            <div className="lg:w-3/4 p-8 flex flex-col justify-between">
              ...
              <div className="mt-8 border-t pt-4">
                <p className="text-xs text-gray-500 leading-relaxed">
                  <strong>Disclaimer:</strong> FinEd is NOT affiliated with or
                  endorsed by any bank listed above. The data is based on public
                  information from official sources for educational purposes
                  only. Please verify all financial decisions directly with the
                  bank.
                </p>
              </div>
            </div>
...
export default SimplySaveCardList;
